...
// Funkcje API dla inspekcji (CrowdScan)
export const inspectionAPI = {
  // Zgłoś inspekcję (dla użytkowników)
  createInspection: async (inspectionData) => {
    try {
      const response = await api.post('/inspections', inspectionData);
      return response.data;
    } catch (error) {
      console.error('Błąd przy zgłaszaniu inspekcji:', error);
      throw error;
    }
  },

  // Pobierz zgłoszenia oczekujące na weryfikację (dla inspektorów)
  getQueuedInspections: async () => {
    try {
      const response = await api.get('/inspections/queued');
      return response.data;
    } catch (error) {
  ...
  // Zweryfikuj zgłoszenie (dla inspektorów)
  verifyInspection: async (id, status) => {
    try {
      const response = await api.put(`/inspections/${id}`, { status });
      return response.data;
    } catch (error) {
      console.error('Błąd przy weryfikacji zgłoszenia:', error);
      throw error;
    }
  },

  // Pobierz reputację użytkownika
