-- Migration: Fix charging tables for BIGINT users.id (SAFE VERSION)
-- Date: 2025-10-28
-- This version clears existing data to avoid type conversion issues
-- Use this when your users.id is BIGINT (not UUID)

-- ========================================
-- OPTION A: If you want to KEEP existing data
-- ========================================
-- Skip this migration and manually export/import your data

-- ========================================
-- OPTION B: If you want to START FRESH (recommended for development)
-- ========================================

-- Step 1: Drop all charging tables (this will delete all charging data!)
...
-- Step 2: Recreate tables with correct types
-- IMPORTANT: user_id and owner_id are BIGINT to match users.id
-- Other IDs (station, session, etc.) remain UUID
...
DO $$
BEGIN
    RAISE NOTICE '✅ Migration completed: Charging tables recreated with BIGINT user references';
    RAISE NOTICE '⚠️  All previous charging data has been deleted';
    RAISE NOTICE 'ℹ️  users.id type: BIGINT (matching your existing schema)';
END $$;
